...
#endif
#ifndef T
#define T 0.0002             // Sampling Period (5kHz)
#endif
...
int main(void)
{
    ...
    for (j=0; j<20000000; j++) {}  // wait 1s for settle
...
interrupt void TimerISR(void)
{
    ...
    u = -K11*x1hat - K12*x2hat - K2*sigma;
    // anti-windup protection
    if (abs(u) > (float32) 0.99*VIN) {
       u = abs(u)/u * 0.99*VIN;  // u = sgn(u) * VIN
    }

    // Update State
    sigma = sigma + T * (y - r);
    x1hat = (float32) x1hat + T * (x2hat - L1*(x1hat - y));
    x2hat = (float32) x2hat + T * (-A*x2hat + B*u - L2*(x1hat - y));

    // Convert to 3-phase representation
    alpha_v = abs(u);
    if (u >= 0) {
        phi_v = PI/2;
    } else {
    ...
    vA = (VIN/2)+sqrt(2.0/9)*alpha_v*cos(N*y+phi_v-PI/6);
    vB = (VIN/2)+sqrt(2.0/9)*alpha_v*cos(N*y+phi_v-PI/6-2*PI/3);
    vC = (VIN/2)+sqrt(2.0/9)*alpha_v*cos(N*y+phi_v-PI/6+2*PI/3);
    ...
    // Log Data
    // record every 5th sample (1kHz sampling)
    if (ISR_count % 5 == 0 && i < LOG_LENGTH) {
        u_log[i] = u;
        y_log[i] = y;
        i++;
    }
